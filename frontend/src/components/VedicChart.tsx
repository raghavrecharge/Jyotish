...
const VedicChart: React.FC<Props> = ({ 
  ...
  return (
    <div className="w-full bg-white dark:bg-slate-900 rounded-[40px] border border-[#f1ebe6] dark:border-slate-800 shadow-sm overflow-hidden flex flex-col group/chart transition-colors">
      {/* Chart Header/Toolbar */}
      <div className="px-6 py-4 border-b border-[#f1ebe6] dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50/50 dark:bg-slate-950/30">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center bg-white dark:bg-slate-800 border border-[#f1ebe6] dark:border-slate-700 rounded-xl p-1 shadow-sm">
            <button 
              onClick={zoomOut}
              className="p-2 hover:bg-orange-50 dark:hover:bg-orange-950/20 text-slate-400 hover:text-orange-500 rounded-lg transition-all active:scale-90"
              title="Zoom Out"
            >
            ...
            <button 
              onClick={resetZoom}
              className="px-2 text-[10px] font-black text-slate-600 dark:text-slate-300 hover:text-orange-500 transition-colors"
              title="Reset Zoom"
            >
      ...
      {/* Main Chart Content */}
      <div className="p-4 sm:p-8 flex items-center justify-center bg-[#fffaf5] dark:bg-slate-950 relative overflow-hidden min-h-[400px] transition-colors">
        ...
        <div className="w-full transition-transform duration-500 ease-out flex justify-center">
          {style === 'North' ? (
            <NorthIndianChart 
              chart={chart}
              // Fixed: replaced non-existent selectedPoint with selectedPlanet from props
      ...
      {/* Chart Footer Info */}
      <div className="px-6 py-3 bg-white dark:bg-slate-900 border-t border-[#f1ebe6] dark:border-slate-800 flex items-center justify-between">
        <div className="flex items-center gap-4 text-[9px] font-bold text-slate-400 dark:text-slate-600 uppercase tracking-widest">
