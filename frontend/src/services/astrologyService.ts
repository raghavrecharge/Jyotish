...
export const astrologyService = {
  ...
  calculateNatalChart(birthData: BirthData): DivisionalChart {
    ...
    const isStelliumTest = birthData.name.toLowerCase().includes('stellium');
    ...
    const points: ChartPoint[] = Object.values(Planet).map((p, i) => {
      ...
      // FORCING 5 PLANETS IN 6th HOUSE IF NAME IS "Stellium"
      let house = ((Math.floor(seed * (i + 5)) % 12) + 1);
      if (isStelliumTest && [Planet.Sun, Planet.Moon, Planet.Mars, Planet.Mercury, Planet.Jupiter].includes(p)) {
        house = 6;
      }
